...
func discoverMixins() (map[string]*mixinDefinition, error) {
	...
	// Load actual fields using Ent's schema parser.
	if len(result) > 0 {
...
//
// We use Ent's graph parser indirectly by loading the generated graph and
// matching the field definitions against mixin names. Since mixin fields are
// part of the graph's resolved fields, we identify fields by reading the
// mixin Fields() methods from source.
func loadMixinFields(
	...
	for _, entry := range entries {
		...
		for _, decl := range file.Decls {
			...
			// Field names are extracted directly from field.String(),
			// field.Enum(), field.Int(), etc.
...
func renderMixin(
	...
	// Mixin field discovery from the source gives us the names, while the
	// resolved graph is used later for concrete field types.
	//
	// Since this function only receives source-derived field names, primitive
	// inference is handled conservatively here.
	//
	// The common mixins (ID, Time, Job, BaseHash) are resolved explicitly
	// below.
...
func renderEntity(
	...
	for _, mixinName := range appliedMixins {
		...
		validMixins = append(validMixins, mixinName)
	}
